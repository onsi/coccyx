var NamedCustomView = Coccyx.root.View.extend({
...
var UnnamedCustomView = Coccyx.root.View.extend({
  initialize: function() {
    this.initialized = true;
  }
});
  ...
  describe('constructorName', function() {    
    describe('when constructorName is set on protoProperties', function() {
      ...
      it('should work for models, collections, and routers too', function() {
        var model = new (Coccyx.root.Model.extend({constructorName:'MyModel'}))();
        expect(model.constructor.name).toEqual('MyModel');
        
        var collection = new (Coccyx.root.Collection.extend({constructorName:'MyCollection'}))();
        expect(collection.constructor.name).toEqual('MyCollection');

        var router = new (Coccyx.root.Router.extend({constructorName:'MyRouter'}))();
        expect(router.constructor.name).toEqual('MyRouter');
      });
      
      it('should not overwrite a passed in constructor', function() {
        var Grumpy = Coccyx.root.Model.extend({
      ...
      it('should not interfere with subclassing', function() {
        var Dog = Coccyx.root.Model.extend({
          constructorName:'Dog',
          initialize: function() {
            this.species = 'dog'
          }
